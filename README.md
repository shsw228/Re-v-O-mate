...
- [x] M4 — configuration UI (SwiftUI): per-mode LED editor (custom RGB **or** 9 presets) with **live preview (0x63) + persist (flash)**; dial CW/CCW & button action editing across **all SetTypes** (keyboard w/ modifiers, mouse buttons, **mouse move/scroll payloads**, media, joypad…); **per-button script & special-function assignment**; dial/button viewer. Serializer round-trip-validated.
